...
1. обратный прокси (nginx/Caddy) с TLS и **аутентификацией** — собственная
   аутентификация в MVP отсутствует (docs/SECURITY.md п. 6);
2. ограничение доступа сетью суда (VPN/инtranet);
3. регулярное резервное копирование `data/store/`;
4. выделенный сервисный пользователь ОС с правами только на каталог данных.

Без выполнения п. 1–2 сетевое развертывание запрещено моделью угроз.
